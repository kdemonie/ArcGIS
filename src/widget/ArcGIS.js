...
return skipCheckDefine("ArcGIS/widget/ArcGIS", [
		/*~♦~♦ Dojo ♦~♦~*/
		"eDojo/global",
		"dojo/_base/declare",
		"dojo/_base/array",
		...
		return declare("ArcGIS.widget.ArcGIS", [_MxWidgetBase, _TemplatedMixin], {
			templateString: "<div class=\"claro clarite\"></div>",
			_progressID: null,
			_handle: null,
			_contextObj: null,
			_gisMap: null,
					...
					if (project) {
						...
						this.geometryService.project(
							...
							function(projectedPoints) {
								if (!this.declarationsGeometry){/* check again, because this is async */
			...
			_zoomToFitDeclarations: function(declarationMFResult) {
				...
				this.geometryService.convexHull(points,
					/*callback=*/geometry => {
						this.declarationsGeometry = geometry;
						if(geometry.getExtent() !== null){
							this._gisMap.setExtent(geometry.getExtent());
						}else{							
							this._gisMap.centerAndZoom(
								geometry,
								Number(this._singleObjectZoom) - 1
